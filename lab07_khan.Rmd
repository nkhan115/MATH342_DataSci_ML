...
#)
#all_angles_cpp(X)
#this code almost destroyed my computer
...
#angle1 <- all_angles(X, 10)
#keeps saying unused argument?
X <- c()
...
#angle_3 = all_angles(X, 1000)

#ggplot() + geom_line(aes(x=angle_1, fill = "black"), alpha = 1) + geom_line(aes(x=angle_2, fill = "green"), alpha = 1) + geom_line(aes(x=angle_1, fill = "blue"), alpha = 1) 
```
...
fib <- function(n, t){
  if (n == 1 | n == 2) return (t)
  else return(fib(n-1, t) + fib(n-2, t))
}
...
```{r}
#cppFunction(
  'nth_fibonnaci_cpp(n, t){
  if (n ==1 | n == 2) return t;
  else return (nth_fibonacci_cpp(n-1, t) + nth_fibonacci_cpp(n-2, t));
  }
  '
#)
```
...
```{r}
#ggplot() + geom_line(aes(y=n, x=log(time), col="time")) + geom_line(aes(y=n, x= log(time_cpp), col="time_cpp"))
```
...
storms %>%
  group_by(name) %>% summarise(max_category = max(category),
                               ...
                               max_diam_ts = max(ts_diameter),
                               max_diam_hu = max(hu_diameter)) %>% na.omit()
```
...
```{r}
storms %>% group_by(name) %>% summarise(lats = mean(lat), long = mean(long))
```

For each storm, summarize its duration in number of hours (to the nearest 6hr increment).

```{r}
storms %>% group_by(name) %>% summarise(duration = (hour) %% 6 == 0)
```
...
```{r}
#storms %>% 
  #storm_number <- c(, range(1:length(storms)))
```
...
```{r}
#storms %<% c(day_of_week = weekdays(timestamp), week_of_year = week(timestamp))
```

For each storm, summarize the day in which is started in the following format "Friday, June 27, 1975".

```{r}
#storms %>% group_by(name) %>% summarise(date = min(timestamp)) %>% 
  #c(date = paste(weekdays(date), paste(months(date), day(date), sep = " "),
                 #year(date), sep=""))
```
...
```{r}
serious_storms %<>% mutate(lat_long = paste(lat, long, sep=" ")) 
```
...
```{r}
storms %>% group_by(name) %>% summarise(x = max(wind), max_pressure = max(pressure))
```
